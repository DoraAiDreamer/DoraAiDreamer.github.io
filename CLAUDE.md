...
- **Static pages at root** — `index.html` (category cards + an inference-column banner), `inference.html` (the `/inference/` 推理加速专栏 landing), `inference_view.html` (the `/inference/view/` markdown viewer), `ml.html` (the `/ml/` 机器学习 chapter — a rich, SVG-figure-driven intro that links the 西瓜书 notes), `about.html`, `tags.html` (tag cloud), `404.html`, `offline.html`, `feed.xml`. `vllm.html` is now just a no-title redirect stub from the old `/vllm/` to `/inference/`. Any root page with a `title:` in its front matter automatically appears in the nav (`nav.html` iterates `site.pages`); the redirect stub and the viewer deliberately omit `title` so they don't show up there.
